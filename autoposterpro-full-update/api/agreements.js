...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'DELETE') {
      ...
      agreement.voidedAt = new Date().toISOString();
      agreement.voidedBy = 'admin';
      agreement.voidReason = reason || 'Voided by administrator';
      
      await kv.set(id, agreement);
      
      // Notify customer that agreement was voided
      try {
        await resend.emails.send({
          from: 'AutoPosterPro <support@autoposterpro.com>',
          to: agreement.contactEmail,
          subject: `Agreement Voided - ${agreement.dealerName}`,
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h2 style="color: #ef4444;">Agreement Voided</h2>
              <p>Hi ${agreement.contactFirstName || agreement.contactName},</p>
              <p>The AutoPosterPro agreement for <strong>${agreement.dealerName}</strong> has been voided.</p>
              ${reason ? `<p><strong>Reason:</strong> ${reason}</p>` : ''}
              <p>If you have any questions, please contact us at support@autoposterpro.com.</p>
              <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
              <p style="color: #666; font-size: 12px;">1553048 B.C. Ltd. dba AutoPosterPro | support@autoposterpro.com</p>
            </div>
          `
        });
      } catch (e) {
        console.error('Failed to send void notification:', e);
      }
      
      return res.status(200).json({ success: true, message: 'Agreement voided' });
    }
